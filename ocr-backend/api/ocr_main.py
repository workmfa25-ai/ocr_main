...
# def convert_pdf_to_text_doctr(file_path: str, model):
...
#         word_confs = [w.confidence for b in page.blocks for l in b.lines for w in l.words]
...
# # === Load embeddings ===
# def load_embeddings():
#     return LocalHFEmbeddings("/home/ocr/Downloads/ocr_backend/sentence_transformer_local")
...
#     response = chat(model="mistral", messages=[{"role": "user", "content": prompt_text}])
...
#         model="mistral",
#         messages=[
#             {"role": "system", "content": "Correct OCR spelling and spacing issues without changing meaning."},
...
#     json.dump([{"page": p, "confidence": c} for p, c in page_confidences], open("page_confidences.json", "w"))

#     chunks = list(split_into_chunks(open(ocr_file_path, "r", encoding="utf-8").read()))
#     corrected_output_path = "corrected_text.txt"
#     with open(corrected_output_path, "w", encoding="utf-8") as f_out:
#         for chunk in chunks:
#             f_out.write(correct_spelling_with_mistral(chunk) + "\n\n")

#     splitter = RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=20)
#     docs = splitter.create_documents([open(corrected_output_path, "r", encoding="utf-8").read()])
